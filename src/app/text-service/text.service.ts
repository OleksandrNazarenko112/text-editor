...
export class TextService {
  private textId = new BehaviorSubject('');
  ...
  getText(): Observable<any> {
    return this.http.get( API_URL + 'text');
  }
  putText(text): Observable<any> {
    return this.http.put( API_URL + 'text', text);
  }
  getTextId() {
    return this.textId.asObservable();
  }
  setTextId(id) {
    this.textId.next(id);
  }
